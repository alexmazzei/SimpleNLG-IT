 ...
 */
package simplenlg.orthography.italian;
		...
		for (int index = 0; index < length; index++) {
			...
			if ( DiscourseFunction.CONJUNCTION.equals(realisedChild
				...
				boolean repeated = realisedChild
					.getFeatureAsBoolean(ItalianLexicalFeature.REPEATED_CONJUNCTION);
				if (index == 0) {
					if (repeated) {
					...
					if (!realisedChild.getFeatureAsBoolean(ItalianLexicalFeature.NO_COMMA)) {
					realisation.append(", "); //$NON-NLS-1$
					}
					realisedChild = realisedChild.realiseOrthography();
					realisation.append(realisedChild.getRealisation());
		...
		for (int i = 0; i < components.size(); i++) {
			...
			if (childRealisation != null && !childRealisation.isEmpty() ) {
				...
				if ( !separatorAdded && (DiscourseFunction.FRONT_MODIFIER.equals(function)
						...
						&& !thisElement.getFeatureAsBoolean(ItalianLexicalFeature.NO_COMMA)) {
					realisation.append(",");
					separatorAdded = true;
				}
				
				// join imperative verb and its clitic complements
				...
				if (!separatorAdded && i < components.size() - 1) {
					...
					if (imperative
							...
							&& nextElement.getFeatureAsBoolean(ItalianInternalFeature.CLITIC)) {
						//System.out.println("verbo clitico appeso alla fine");
						//Cristina B. no - for clitics after the verb 
						if (lastChar != '\'') {
							realisation.append("");
							dashAdded = true;
						}
